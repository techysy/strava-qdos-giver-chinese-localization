...
| 主界面 | 高级选项 |
|--------|----------|
| ![中文界面 - 主界面](png/on%20Strava%20Dashboard%2001.png) | ![中文界面 - 高级选项](png/on%20Strava%20Dashboard%2002.png) |

### 🔧 诊断与提示

| 运行诊断（统计页面活动卡片） | 错误提示（非仪表板页面） |
|----------------------------|------------------------|
| ![运行诊断](png/Run%20Diagnostics.png) | ![未在 Strava 仪表盘](png/Not%20on%20Strava%20Dashboard.png) |

- **运行诊断**：统计当前页面活动卡片数量、点赞按钮、作者信息等，用于排查问题
- **错误提示**：当用户不在 Strava 仪表盘页面时显示，引导用户前往正确页面
...
1. **中文时间格式支持** - 添加对中文界面时间显示的解析支持：
   - 支持 `刚刚`、`今天`、`昨天`
   - 支持 `X分钟前`、`X小时前`、`X天前` 等格式

2. **点赞计数修复** - 修复点赞成功但计数显示为0的问题：
   - 增强 `alreadyKudoed()` 函数，支持多种检测方式
   - 改进 `clickWithRetry()` 函数，增加多次验证重试
   - 添加最终回退机制，确保点赞被正确计数

3. **界面预览更新** - 更新演示图片，展示中文界面效果
...
- **popup.html**：所有界面文本
- **popup.js**：动态状态消息
- **kudos-engine.js**：引擎运行时消息 + 中文时间解析 + 点赞检测增强
